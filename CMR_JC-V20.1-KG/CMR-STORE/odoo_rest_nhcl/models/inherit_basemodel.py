...
class BaseModel(models.AbstractModel):
    ...
    def _read_format(self, fnames, load='_classic_read'):
        try:
            is_rest_api = request.context.get("odoo_rest_api", False)
        except RuntimeError:
            # When request is unbound, we're not in an HTTP context.
            is_rest_api = False
        ...
        if is_rest_api:
            ...
            return super(BaseModel, self)._read_format(fnames=fnames, load=load)
