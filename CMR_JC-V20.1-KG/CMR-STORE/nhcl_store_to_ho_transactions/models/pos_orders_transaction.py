...
class PosOrder(models.Model):
    ...
    def get_pos_customer_invoice(self):
                ...
                try:
                    ...
                    for line in entry.invoice_line_ids:
                        if line.quantity >= 1:
                            ...
                            product_url = f"http://{ho_ip}:{ho_port}/api/product.product/search?domain={product_domain}&fields=['id','name']"
                            ...
                            if line.tax_ids:
                                if entry.journal_id.company_id.state_id.name != 'Andhra Pradesh':
                                    ...
                                    tax_url = f"http://{ho_ip}:{ho_port}/api/account.tax/search?domain={tax_domain}&fields=['id','name']"
                                    ...
                                    parent_tax_url = f"http://{ho_ip}:{ho_port}/api/account.tax/search?domain={parent_tax_domain}&fields=['id','name']"
                            ...
                            invoice_lines.append((0, 0, {
                                ...
                                "price_subtotal": line.price_subtotal,
                                "quantity": line.quantity,
                                # "discount_amount": line.discount_percentage,
                                "discount": discount,
                            }))
                    ...
                    # # -------------------- POS Order --------------------
                    # pos_domain = [
                    #     ('name', '=', entry.pos_order_ids.name),
                    #     ('company_id', '=', company_id[0]['id'])
                    # ]
                    #
                    # pos_url = f"http://{ho_ip}:{ho_port}/api/pos.order/search?domain={pos_domain}&fields=['id','name']"
                    #
                    ...
                    #
                    # pos_id = pos_data.get("data", [])

                    # if not pos_id:
                    #     msg = f"POS Order Not Found : {entry.pos_order_ids.name}"
                    #     ho_id.create_cmr_transaction_replication_log(
                    #         'Customer Invoice',
                    #         entry.id,
                    #         entry.name,
                    ...
                    if not move_id:
                        ...
                        move_vals = {
                            ...
                            "ref": entry.name,
                            "pos_bill_ref": entry.pos_bill_ref,
                            "invoice_date": entry.invoice_date.strftime("%Y-%m-%d"),
                            "invoice_date_due": entry.invoice_date_due.strftime("%Y-%m-%d"),
                            "move_type": entry.move_type,
                            # "journal_id": account_journal[0]['id'],
                            "journal_id": 484,
                            "amount_total": entry.amount_total,
            ...
            for order in store_pos_delivery_orders:
                try:
                    ...
                    payload = [{
                        ...
                        'origin': order.name,
                        'scheduled_date': order.date_done,
                        'location_id': location_id,
                        'location_dest_id': dest_id,
                        'company_id': company_id,
                        'pos_order_id': pos_id,
