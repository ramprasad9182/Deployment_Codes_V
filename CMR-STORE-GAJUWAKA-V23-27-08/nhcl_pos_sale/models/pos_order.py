...
class PosOrder(models.Model):
    ...
    def update_credit_voucher_to_customer(self, partner_id=None):
        ...
        if not partner.exists():
            return
        ho_id = self.env['nhcl.ho.store.master'].search([('nhcl_store_type', '=', 'ho'), ('nhcl_active', '=', True)], limit=1)
        if ho_id and ho_id.nhcl_terminal_ip and ho_id.nhcl_port_no:
            ...
            try:
                ...
                customer_response = requests.get(customer_url, headers=headers_source, timeout=(1, 1.5))
                if customer_response.status_code == 200:
                    ...
                    if customer_data and len(customer_data) > 0:
                        ...
                        if ho_partner_id:
                            ...
                            cn_response = requests.get(customer_credit_notes_url, headers=headers_source, timeout=(1, 1.5))
                            if cn_response.status_code == 200:
                                ...
                                for each in customer_credit_notes_data:
                                    ...
                                    if voucher_num and voucher_num not in existing_vouchers:
                                        vouchers_to_create.append({
                                            'voucher_number': voucher_num,
                                            'pos_bill_number': each.get('pos_bill_number'),
                                            'pos_bill_date': each.get('pos_bill_date'),
                                            'total_amount': each.get('total_amount', 0.0),
                                            'deducted_amount': each.get('deducted_amount', 0.0),
...
class PosPayment(models.Model):
    ...
    def action_reverse_je(self):
        ...
        """
        for payment in self:
            # for session close warning before reverse entry
            ...
            session_name = payment.pos_order_id.session_id.name if payment.pos_order_id.session_id else ''

            # Build reason safely (no extra "/" if session missing)
            reason = _(
