...
class PosXReport(models.TransientModel):
    ...
    pos_session_ids = fields.Many2many('pos.session', 'pos_sessions', string="POS Session(s)",
                                       domain="[('state', 'in', ['opened'])]", required=True)
    ...
    def generate_x_report(self):
        ...
        return self.env.ref('nhcl_bi_pos_reports.action_x_report_print').report_action([], data=data)


class OpenSessionReport(models.AbstractModel):
    _name = 'report.nhcl_bi_pos_reports.report_open_session'
