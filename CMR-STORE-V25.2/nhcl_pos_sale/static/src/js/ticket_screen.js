...
//        this.delete_order_access = this.pos.user.allow_delete_order_button;
    ...
    async onDeleteOrder(order) {
        const res = await super.onDeleteOrder(...arguments);
        await this.pos._removeOrdersFromServer();
        return res;
    },

    async onClickDeleteSelectedOrders() {
        const ordersToDelete = this.getFilteredOrderList().filter(order => order.select_order_in_ticket_screen);
    ...
    _getToRefundDetail(orderline) {
        ...
        if (res && res.orderline && orderline) {
            res.orderline.employe_no = orderline.get_emp_no ? orderline.get_emp_no() : (orderline.employe_no || orderline.empNo || "");
            res.orderline.badge_id = orderline.get_badge_id ? orderline.get_badge_id() : (orderline.badge_id || orderline.badge || "");
            res.orderline.employ_id = orderline.get_employe_id ? orderline.get_employe_id() : (orderline.employ_id || orderline.empId || false);
        }
        return res;
    },
    ...
    _prepareRefundOrderlineOptions(toRefundDetail) {
        ...
        if (toRefundDetail && toRefundDetail.orderline) {
            options.employe_no = toRefundDetail.orderline.employe_no;
            options.badge_id = toRefundDetail.orderline.badge_id;
            options.employ_id = toRefundDetail.orderline.employ_id;
        }
        return options;
    },
});
