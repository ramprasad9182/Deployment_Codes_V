...
patch(Order.prototype, {
    ...
    async add_product(product, options) {
        ...
        const orderline = result || this.get_selected_orderline();
        if (orderline && !orderline.is_reward_line && !orderline.is_fix_discount_line) {
            let unitPrice = typeof orderline.get_unit_price === 'function' ? orderline.get_unit_price() : orderline.price;
            if (unitPrice === undefined || unitPrice === null || isNaN(unitPrice)) {
                if (orderline.pack_lot_lines && orderline.pack_lot_lines.length > 0) {
                    let k;
                    orderline.pack_lot_lines.forEach((pack) => { k = pack.lot_name; });
                    const stockLot = this.pos?.stock_lots_by_name ? this.pos.stock_lots_by_name[k] : null;
                    if (stockLot && stockLot.stockLot && stockLot.stockLot.rs_price !== undefined) {
                        unitPrice = stockLot.stockLot.rs_price;
                    }
                }
                if (unitPrice === undefined || unitPrice === null || isNaN(unitPrice)) {
                    unitPrice = (options && options.price !== undefined) ? options.price : (orderline.price ?? 0);
                }
            }
            if (unitPrice <= 0) {
                const productName = (typeof orderline.get_full_product_name === 'function' ? orderline.get_full_product_name() : false) || orderline.product?.display_name || product?.display_name || "scanned item";
                this.remove_orderline(orderline);
                await this.pos.env.services.popup.add(ErrorPopup, {
                    title: _t("0 RSP Price Not Allowed"),
                    body: _t(`${productName} has 0 rsp price and that not allowed in pos`),
                });
                return false;
            }
        }
    ...
    set_orderline_options(orderline, options) {
        ...
        if (options) {
            if (options.employe_no !== undefined || options.empNo !== undefined) {
                orderline.set_emp_no(options.employe_no !== undefined ? options.employe_no : options.empNo);
            }
            if (options.badge_id !== undefined || options.badge !== undefined) {
                orderline.set_badge_id(options.badge_id !== undefined ? options.badge_id : options.badge);
            }
            if (options.employ_id !== undefined || options.empId !== undefined) {
                orderline.set_employee_id(options.employ_id !== undefined ? options.employ_id : options.empId);
            }
        }
    },

    get_custom_display_total() {
        const totalWithTax = this.get_custom_totalwithtax();
        const displayTotal = totalWithTax - (this.credit_note_amount || 0);
        if (totalWithTax < 0) {
            return displayTotal;
        }
    ...
    _resetPrograms() {
        ...
        for (const line of this.get_orderlines()) {
            ...
            line.discount_reward = false;
            line.set_discount(0);
            line.set_discount_reward(false);
            line.is_reward_line = false;
            line.promo = 0;
            line.reward_product_id = false;
            line.reward_product_name = false;
            line.coupon_id = null;
            line.points_cost = 0;
            line.reward_identifier_code = null;
            line.discount_value = 0;
            line.discount_minimum_amount = 0;
        }
    ...
    _applyReward(reward, coupon_id, args) {
        ...
        if (result === true && (reward.reward_type === "product" || reward.reward_type === "discount_on_product")) {
            const order = this.pos.get_order();
            if (reward.stock_lot_ids && (reward.stock_lot_ids.size > 0 || reward.stock_lot_ids.length > 0)) {
                const lotSet = reward.stock_lot_ids instanceof Set ? reward.stock_lot_ids : new Set(reward.stock_lot_ids);
                order.get_orderlines().forEach(line => {
                    if (line.pack_lot_lines && line.pack_lot_lines.length) {
                        ...
                        if (stockLot && lotSet.has(stockLot.stockLot.id)) {
