        ...
        if (this.props.title == "Lot/Serial Number(s) Required") {
            ...
            for (let eachItem of lot_names) {
                ...
                if (lot) {
                    ...
                    if (lot.stockLot && lot.stockLot.is_used && (!lot.stockLot.product_qty_pos || lot.stockLot.product_qty_pos <= 0)) {
                        this.popup.add(ErrorPopup, {
                            title: _t("Serial Number Already Used"),
                            body: _t(`The Lot/Serial Number ${lot.stockLot.name} is already used/sold.`),
                        });
                        return false;
                    }
                    if (!lot.stockLot.product_qty_pos || lot.stockLot.product_qty_pos <= 0) {
                        this.popup.add(ErrorPopup, {
                            title: _t("No Stock Available"),
                            body: _t(`No stock available for Lot/Serial Number: ${lot.stockLot.name}`),
                        });
                        return false;
                    }
                    if (lot.stockLot && lot.stockLot.location_id && lot.stockLot.location_id[0]) {
                        const lot_damage_return_location = (this.pos.stock_location || []).filter(
                            (l) =>
                                l.id === lot.stockLot.location_id[0] &&
                                l.cmr_location_type &&
                    ...
                    if (!lot.stockLot.sale_tax_ids || lot.stockLot.sale_tax_ids.length === 0) {
                        this.popup.add(ErrorPopup, {
                            title: _t("Missing Taxes on Lot/Serial Number"),
                            body: _t(`No taxes are configured for the Lot/Serial Number: ${lot.stockLot.name}`),
                        });
                        return false;
                    }
                    const lotId = lot.stockLot.id;
                    const programsWithLot = this.pos.getProgramsForLot(lotId);
                    if (programsWithLot.length > 1) {
                        this.popup.add(ErrorPopup, {
                            title: _t("Multiple Loyalty Programs"),
                            body: _t(`Barcode ${lot.stockLot.ref} is configured in multiple loyalty programs:\n • ${programsWithLot.join(' • ')}.\nScanning/entering is not allowed.`),
                        });
                        return false;
                    }
                        ...
                        if (product) {
                            ...
                            if (lot) {
                                ...
                                if (firstValue.stockLot.product_id && firstValue.stockLot.product_id[0]) {
                                    if (!firstValue.stockLot.sale_tax_ids || firstValue.stockLot.sale_tax_ids.length === 0) {
                                        this.popup.add(ErrorPopup, {
                                            title: _t("Missing Taxes on Lot/Serial Number"),
                                            body: _t(`No taxes are configured for the Lot/Serial Number: ${firstValue.stockLot.name}`),
                                        });
                                        return false;
                                    }
                                    const lotId = firstValue.stockLot.id;
                                    const programsWithLot = this.pos.getProgramsForLot(lotId);
                                    if (programsWithLot.length > 1) {
                                        this.popup.add(ErrorPopup, {
                                            title: _t("Multiple Loyalty Programs"),
                                            body: _t(`Barcode ${firstValue.stockLot.ref} is configured in multiple loyalty programs:\n • ${programsWithLot.join(' • ')}.\nScanning/entering is not allowed.`),
                                        });
                                        return false;
                                    }
                                    const product = this.pos.db.get_product_by_id(
