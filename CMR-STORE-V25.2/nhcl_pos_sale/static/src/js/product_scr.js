...
patch(ProductScreen.prototype, {
    ...
    async _barcodeProductActionLocked(code) {
        ...
        // Check if the scanned code matches a specific Lot Name that is already sold
        if (code && code.base_code) {
            try {
                const exactLotMatches = await this.orm.call('stock.lot', 'search_read', [
                    [['name', '=', code.base_code]],
                    ['id', 'name', 'ref', 'rs_price', 'is_under_plan', 'sale_tax_ids', 'product_qty_pos', 'location_id', 'is_used', 'product_id']
                ]);
                if (exactLotMatches && exactLotMatches.length > 0) {
                    if (exactLotMatches[0].is_used && (!exactLotMatches[0].product_qty_pos || exactLotMatches[0].product_qty_pos <= 0)) {
                        return await this.popup.add(ErrorPopup, {
                            title: _t("Already Sold Lot / Serial Number"),
                            body: _t(`Serial Number '${exactLotMatches[0].name}' has already been sold and used.`),
                        });
                    }
                    if (exactLotMatches[0].sale_tax_ids && exactLotMatches[0].sale_tax_ids.length > 0) {
                        const hasIgst = exactLotMatches[0].sale_tax_ids.some(taxId => {
                        ...
                        if (hasIgst) {
                            return await this.popup.add(ErrorPopup, {
                                ...
                                body: _t(`Serial Number '${exactLotMatches[0].name}' has IGST tax applied.`),
                            });
                        }
                    }
                }
            } catch (e) {
                console.error("Error checking exact lot sold status:", e);
            }
        }

        let barcode = this.env.services.pos.barcode_by_name[code.base_code];
        if (!barcode) {
            let product_found = this.env.services.pos.db.get_product_by_barcode(code.base_code);
            if (product_found) {
                barcode = {
                    barcode: code.base_code,
                    product_id: [product_found.id, product_found.display_name],
                };
            }
        }
        if (barcode) {
            ...
            if (barcode.product_id) {
                ...
                if (product) {
                    ...
                    if (product.tracking == 'serial' || product.tracking == "lot") {
                        ...
                        try {
                            const brander_serial_nos = await this.orm.call('stock.lot', 'search_read', [domain, ['id', 'name', 'ref', 'rs_price', "mr_price", "is_under_plan", 'product_qty_pos', 'type_product', 'location_id', 'sale_tax_ids', 'product_id']]);
                            ...
                            if (brander_serial_nos && brander_serial_nos.length > 0) {
                                const distinctProductIds = new Set(brander_serial_nos.map(l => l.product_id ? l.product_id[0] : null).filter(Boolean));
                                if (distinctProductIds.size > 1) {
                                    const { confirmed, payload: selectedLot } = await this.popup.add(DuplicateLotSelectionPopup, {
                                        title: _t("Select Matching Item / Variant"),
                                        formattedValue: barcode.barcode,
                                        lots: brander_serial_nos,
                                    });
                                    if (!confirmed || !selectedLot) {
                                        return;
                                    }
                                    first_fifo = selectedLot;
                                } else {
                            ...
                            if (first_fifo) {
                                if (first_fifo.product_id && first_fifo.product_id[0]) {
                                    const lotProduct = this.env.services.pos.db.get_product_by_id(first_fifo.product_id[0]);
                                    if (lotProduct) {
                                        product = lotProduct;
                                    }
                                }
                                ...
                                if (first_fifo.sale_tax_ids && first_fifo.sale_tax_ids.length > 0) {
                                    const hasIgst = first_fifo.sale_tax_ids.some(taxId => {
                                        const tax = this.pos.taxes_by_id[taxId];
                                        return tax && (
                                            (tax.tax_group_id && Array.isArray(tax.tax_group_id) && tax.tax_group_id[1].toUpperCase().includes('IGST')) ||
                                            (tax.name && tax.name.toUpperCase().includes('IGST'))
                                    ...
                                    if (hasIgst) {
                                        return await this.popup.add(ErrorPopup, {
                                            title: _t("IGST Tax Error"),
                                            body: _t(`Serial Number '${first_fifo.name}' has IGST tax applied.`),
                                        });
                                    }
                                }
                                const lotId = first_fifo.id;
                                const programsWithLot = this.pos.getProgramsForLot(lotId);
                                if (programsWithLot.length > 1) {
                                    return await this.popup.add(ErrorPopup, {
                                        title: _t("Multiple Loyalty Programs"),
                                        body: _t(`Barcode ${first_fifo.ref} (Serial Number ${first_fifo.name}) is configured in multiple loyalty programs:\n • ${programsWithLot.join('\n • ')}.\nScanning is not allowed to prevent system conflicts.`),
                                    });
                                }
                            }
                        } catch (error) {
                    ...
                    if (first_fifo) {
                        ...
                        const options = {
                            draftPackLotLines: {
                                ...
                                newPackLotLines: [{ lot_name: first_fifo.name }],
                            },
                            modifiedPackLotLines: {},
                            newPackLotLines: [{ lot_name: first_fifo.name }],
                            price: first_fifo.rs_price,
                            merge: false,
                        };
                        ...
                        const addRes = await selectedOrder.add_product(product, options);
                        if (addRes !== false) {
                            this.currentOrder._updateRewards();
                ...
                if (list.length == 1) {
                    const addRes = await selectedOrder.add_product(list[0], { quantity: 1 });
                    if (addRes !== false) {
                        this.currentOrder._updateRewards();
                        await this.set_sales_employee();
                    }
            ...
            const lineCountBefore = this.currentOrder ? this.currentOrder.get_orderlines().length : 0;
            await super._barcodeProductAction(code);
            const lineCountAfter = this.currentOrder ? this.currentOrder.get_orderlines().length : 0;
            const currentLine = this.currentOrder ? (this.currentOrder.get_selected_orderline() || this.currentOrder.get_orderlines().at(-1)) : null;
            if (lineCountAfter > lineCountBefore || (currentLine && !currentLine.get_emp_no())) {
                await this.set_sales_employee();
            }
        }
    },
    ...
    async _barcodeGS1ActionLocked(parsed_results) {
        ...
        try {
            ...
            if (unbranded_serial_number && unbranded_serial_number.length > 0) {
                ...
                if (programsWithLot.length > 1) {
                    return await this.popup.add(ErrorPopup, {
                        title: _t("Multiple Loyalty Programs"),
                        body: _t(`Barcode ${unbranded_serial_number[0].ref} (Serial Number ${unbranded_serial_number[0].name}) is configured in multiple loyalty programs:\n • ${programsWithLot.join('\n • ')}.\nScanning is not allowed to prevent system conflicts.`),
                    });
                }
            }
        ...
        if (scannedCode && freshLotNames.includes(scannedCode)) {
            ...
            if (product.tracking === 'serial' && product.nhcl_product_type === 'unbranded') {
                ...
                const addRes = await this.currentOrder.add_product(product, {
                    ...options,
            ...
            const addRes = await this.currentOrder.add_product(product, { ...options, ...customProductOptions });
            if (addRes !== false) {
                this.numberBuffer.reset();
                this.currentOrder._updateRewards();
                await this.set_sales_employee();
    ...
    async applicable_rewards_popup() {
        ...
        for (const program of this.pos.programs) {
            if (this.currentOrder._programIsApplicable(program)) {
                if (program.rules.filter((rule) => {
                    if (rule.serial_ids && rule.serial_ids.size > 0) {
                        return lot_ids[0] && rule.serial_ids.has(lot_ids[0]);
                    }
                    return rule.any_product || (rule.valid_product_ids && rule.valid_product_ids.has(selectedline.product.id));
                }).length > 0) {
