...
patch(PaymentScreen.prototype, {
    setup() {
        ...
        onMounted(async () => {
            ...
            if (this.currentOrder && this.currentOrder.get_total_with_tax() < 0) {
                await this.autoSetupRefundPaymentLines();
            }
    ...
    async autoSetupRefundPaymentLines() {
        const order = this.currentOrder;
        if (!order || order.get_total_with_tax() >= 0) return;

        let originalOrderBackendId = order.refunded_order_backend_id;
        if (!originalOrderBackendId) {
            for (const line of order.get_orderlines()) {
                if (line.refunded_orderline_id && this.pos.toRefundLines && this.pos.toRefundLines[line.refunded_orderline_id]) {
                    const detail = this.pos.toRefundLines[line.refunded_orderline_id];
                    if (detail && detail.orderline && detail.orderline.orderBackendId) {
                        originalOrderBackendId = detail.orderline.orderBackendId;
                        break;
                    }
                }
            }
        ...
        if (originalOrderBackendId) {
            try {
                originalPayments = await this.pos.orm.call('pos.order', 'get_order_payment_details', [originalOrderBackendId]);
            } catch (err) {
                console.error("Error fetching original order payments:", err);
            }
        }

        // Clear existing payment lines to set exact original refund payment lines
        while (order.paymentlines.length > 0) {
            order.remove_paymentline(order.paymentlines[0]);
        }

        const refundTotal = order.get_total_with_tax(); // negative value e.g. -1000

        if (originalPayments && originalPayments.length > 0) {
            const origTotalPaid = originalPayments.reduce((sum, p) => sum + p.amount, 0);
            order.allowed_refund_payment_method_ids = originalPayments.map(p => p.payment_method_id);

            for (const p of originalPayments) {
                const pm = this.pos.payment_methods.find(m => m.id === p.payment_method_id);
                if (pm) {
                    const ratio = origTotalPaid !== 0 ? (p.amount / origTotalPaid) : (1 / originalPayments.length);
                    const lineAmount = Math.round(refundTotal * ratio);
                    const newPaymentLine = order.add_paymentline(pm);
                    if (newPaymentLine) {
                        newPaymentLine.set_amount(lineAmount);
                    }
                }
            }
        } else {
            // Fallback if no original payments found: use first configured method
            if (this.payment_methods_from_config && this.payment_methods_from_config.length > 0) {
                const pm = this.payment_methods_from_config[0];
                order.allowed_refund_payment_method_ids = [pm.id];
                const newPaymentLine = order.add_paymentline(pm);
                if (newPaymentLine) {
                    newPaymentLine.set_amount(refundTotal);
                }
            }
        }

        if (order.paymentlines.length > 0) {
            this.selectPaymentLine(order.paymentlines[0].cid);
        }
    },
    ...
    async upiNewPaymentLine(paymentMethod) {
        ...
        if (order.get_total_with_tax() < 0) {
            const allowedIds = order.allowed_refund_payment_method_ids || [];
            if (allowedIds.length > 0 && !allowedIds.includes(paymentMethod.id)) {
                this.popup.add(ErrorPopup, {
                    title: _t("Invalid Payment Method"),
                    body: _t("Only the exact original payment method can be used for refund orders."),
                });
                return false;
            }
            if (order.paymentlines.length >= (order.allowed_refund_payment_method_ids?.length || 1) && Math.abs(order.get_due()) < 0.001) {
                this.popup.add(ErrorPopup, {
                    title: _t("Restricted Action"),
                    body: _t("Refund payment line is already added with exact refund amount."),
                });
                return false;
            }
        }
    ...
    deletePaymentLine(cid) {
        ...
        if (order && order.get_total_with_tax() < 0) {
            this.popup.add(ErrorPopup, {
                title: _t("Restricted Action"),
                body: _t("Payment line cannot be removed for refund orders."),
            });
            return false;
        }
    ...
    updateSelectedPaymentline(amount = false) {
        ...
        if (order && order.get_total_with_tax() < 0) {
            this.popup.add(ErrorPopup, {
                title: _t("Restricted Action"),
                body: _t("Payment amount cannot be modified for refund orders. It must match the original refund amount."),
            });
        ...
        const old_payment_line_amount = this.selectedPaymentLine?.amount || 0;
        const res = super.updateSelectedPaymentline(...arguments);

        // Overpayment check applies to all payment methods (Cash & Bank) on sale orders (total >= 0).
        // Skipped completely in the case of refund orders (total < 0).
        if (order && order.get_total_with_tax() >= 0) {
            if (amount === false) {
                if (this.numberBuffer.get() === null) {
                    amount = null;
            ...
            if (amount && change > 0) {
                ...
                this.popup.add(ErrorPopup, {
                    title: _t("Payment Error"),
                    body: _t("You can't set more than remaining payment!"),
                });
                return;
            }
        }
    ...
    async validateOrder(isForceValidate) {
        ...
        if (order && order.get_total_with_tax() < 0) {
            ...
            if (allowedIds.length > 0) {
                const invalidLine = order.paymentlines.find(line => !allowedIds.includes(line.payment_method.id));
                if (invalidLine) {
                    await this.popup.add(ErrorPopup, {
                        title: _t("Validation Error"),
                        body: _t(`Payment method '${invalidLine.payment_method.name}' is not allowed for this refund order. Must use exact original payment method.`),
                    });
                    return false;
                }
            }
            if (Math.abs(order.get_due()) > 0.001) {
                await this.popup.add(ErrorPopup, {
                    title: _t("Validation Error"),
                    body: _t("Refund payment amount must exactly match the refund order total."),
                });
                return false;
            }
        }
