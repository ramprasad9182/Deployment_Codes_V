...
class PosSession(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if config_id:
                ...
                if config and (not config.cash_rounding or not config.rounding_method or float_compare(config.rounding_method.rounding, 1.0, precision_digits=4) != 0):
                    raise UserError(_("This counter rounding is not set to 1. Please set this counter rounding to 1 before opening."))
        return super().create(vals_list)

    def open_frontend_cb(self):
        for session in self:
            if session.config_id and (not session.config_id.cash_rounding or not session.config_id.rounding_method or float_compare(session.config_id.rounding_method.rounding, 1.0, precision_digits=4) != 0):
                raise UserError(_("This counter rounding is not set to 1. Please set this counter rounding to 1 before opening."))
    ...
    def _handle_order_process_fail(self, order, exception, draft):
        try:
            if self.env.cr and not self.env.cr._closed:
                self.env.cr.rollback()
        except Exception:
            pass
        return super()._handle_order_process_fail(order, exception, draft)
    ...
    def _loader_params_loyalty_reward(self):
        ...
        res["search_params"]["fields"].append("stock_lot_ids")
        return res
    ...
    def _loader_params_hr_employee(self):
        ...
        result['search_params']['fields'].extend(
            ['limited_discount', 'allow_global_discount', 'allow_amount_discount', 'allow_selected_lines_discount', 'max_global_discount', 'max_amount_discount', 'max_selected_lines_discount', 'store_manager', 'user_id'])
        return result
