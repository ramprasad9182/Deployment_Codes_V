...
function _appliedProgramIds(order) {
    ...
    for (const line of order.get_orderlines() || []) {
        ...
        if (rid) {
            const r = order.pos.reward_by_id?.[rid];
            const pid = r?.program_id?.id;
            if (pid) ids.add(pid);
        } else if (line.is_reward_line && line.reward_id) {
    ...
    for (const line of order.get_orderlines() || []) {
        ...
        if (rid) {
            ...
            if (r?.program_id?.id === programId) {
                ...
                } else if (r.buy_with_reward_price === "yes") {
                    if (line.pack_lot_lines) {
                        let lotName;
                        line.pack_lot_lines.forEach(pack => { lotName = pack.lot_name; });
                        const stockLot = order.pos.stock_lots_by_name[lotName];
                        if (stockLot && stockLot.stockLot.rs_price > 0) {
                            const rewardUnitPrice = r.reward_price / r.buy_product_value;
                            total += Math.max(0, stockLot.stockLot.rs_price - rewardUnitPrice) * line.get_quantity();
                        }
                    }
                }
            }
...
function _orderBaseAmount(order) {
    ...
    return lines.reduce((s, l) => s + l.get_price_with_tax_before_discount() * l.get_quantity(), 0);
}
        ...
        try {
            ...
            for (const pid of programsToCheck) {
                ...
                const directRewardFullyApplied = (() => {
                    if (!best || best.reward.buy_with_reward_price !== 'no') {
                        return true;
                    }
                    let eligibleLines = [];
                    ...
                    } else if (best.reward.discount_applicability === 'order') {
                        eligibleLines = this.get_orderlines().filter(line =>
                            line.get_quantity() > 0 &&
                            !line.is_reward_line &&
                    ...
                    let discountable = 0;
                    if (best.reward.discount_applicability === 'order') {
                        discountable = eligibleLines.reduce((sum, line) => sum + line.get_price_with_tax_before_discount(), 0);
                    } else if (best.reward.discount_applicability === 'cheapest') {
                        discountable = eligibleLines.reduce((sum, line) => {
                            const linePriceUnit = line.getComboTotalPriceWithoutTax() / line.get_quantity();
                            return sum + linePriceUnit * line.get_quantity();
                        }, 0);
                    } else { // specific
                        discountable = eligibleLines.reduce((sum, line) => sum + line.get_price_with_tax_before_discount(), 0);
                    }
                    ...
                    let maxDiscount = best.reward.discount_max_amount || Infinity;
                    if (best.reward.discount_mode === "per_point") {
                        maxDiscount = Math.min(maxDiscount, best.reward.discount * this._getRealCouponPoints(best.coupon_id));
                    } else if (best.reward.discount_mode === "per_order") {
                        maxDiscount = Math.min(maxDiscount, best.reward.discount);
                    } else if (best.reward.discount_mode === "percent") {
                        maxDiscount = Math.min(maxDiscount, discountable * (best.reward.discount / 100));
                    }

                    const discountFactor = discountable ? Math.min(1, maxDiscount / discountable) : 1;
                    const expectedDiscountPercent = (best.reward.discount || 100) * discountFactor;
                    ...
                    const allHaveCorrectDiscount = eligibleLines.every(line => Math.abs(line.discount - expectedDiscountPercent) < 0.001);
                ...
                if (!best) {
                    ...
                    if (newClaimables.length) {
                        for (const targetLine of this.get_orderlines()) {
                            if (!targetLine.product || targetLine.is_reward_line || targetLine.refunded_orderline_id) {
                                continue;
                            }
                            const targetProdId = targetLine.product.id;

                            // 1. Identify the reward record for the product on this line
                            const matchingClaim = newClaimables.slice().sort((a, b) => {
                                ...
                                if (r?.stock_lot_ids && r.stock_lot_ids.size > 0) {
                                    if (!targetLine.pack_lot_lines || !targetLine.pack_lot_lines.length) {
                                        return false;
                                    }
                                    let lotName;
                                    targetLine.pack_lot_lines.forEach(pack => { lotName = pack.lot_name; });
                                    const stockLot = this.pos.stock_lots_by_name[lotName];
                                    if (!stockLot) return false;
                                    return r.stock_lot_ids.has(stockLot.stockLot.id);
                                }
                                return r?.cmr_discount_product_ids?.includes(targetProdId);
                            });
                            ...
                            if (matchingClaim) {
                                ...
                                if (result && result.length) {
                                    const validPointData = result.find(item => {
                                        ...
                                        const totalAmt = contributingLines.reduce((sum, l) => sum + l.get_price_with_tax_before_discount(), 0);
                                    ...
                                    if (validPointData) {
                                        ...
                                        const usedUses = this.get_orderlines().filter(l =>
                                            l.reward_id === reward.id && l !== targetLine
                                        ).length;

                                        if (usedUses < maxUsesAllowed) {
                                            if (targetLine.reward_id !== reward.id) {
                                                targetLine.reward_id = reward.id;
                                                this._updateRewards();
                                            }
                                        }
                                    }
