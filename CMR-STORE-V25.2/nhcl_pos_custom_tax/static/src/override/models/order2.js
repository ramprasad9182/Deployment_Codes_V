...
patch(Order.prototype, {
    async add_product(product, options) {
        if (this.pos.scanned_qty !== undefined) {
            options = options || {};
            options.quantity = this.pos.scanned_qty;
    ...
    _programIsApplicable(program) {
        ...
        const hasInvalidRule = program.rules.some(rule => {
            const isSerialOrGrc = ['serial', 'grc'].includes(rule.type_filter);
            const hasNoSerials = !rule.serial_ids || (rule.serial_ids instanceof Set ? rule.serial_ids.size === 0 : rule.serial_ids.length === 0);
            return isSerialOrGrc && hasNoSerials;
        });
        if (hasInvalidRule) {
            return false;
        }
        ...
        for (const program of programs) {
            ...
            for (const rule of program.rules) {
                if (rule.mode === "with_code" && !this.codeActivatedProgramRules.includes(rule.id)) {
                 ...
                 const amountWithTax = linesForRule.reduce((sum, line) => sum + line.get_price_with_tax_before_discount(), 0);
                 const amountWithoutTax = linesForRule.reduce((sum, line) => sum + line.get_price_without_tax_before_discount(), 0);
                ...
                for (const line of orderLines) {
                    if (this._cmrLineMatchesRule(rule, line, program) &&
                        ...
                        qtyPerProduct[prodId] = (qtyPerProduct[prodId] || 0) + lineQty;
                         orderedProductPaid += line.get_price_with_tax_before_discount();
                    ...
                    if (rule.reward_point_mode === "unit") {
                        ...
                        for (const line of orderLines) {
                            if (line.is_reward_line || !rule.valid_product_ids.has(line.get_product().id) || line.get_quantity() <= 0 || line.ignoreLoyaltyPoints({ program })) {
                             ...
                             const pointsPerUnit = roundPrecision((rule.reward_point_amount * line.get_price_with_tax_before_discount()) / line.get_quantity(), 0.01);
                            if (pointsPerUnit > 0) {
...
patch(Orderline.prototype, {
    set_quantity(quantity, keep_price) {
        if (this.pos.scanned_qty !== undefined) {
            ...
            quantity = currentQty + this.pos.scanned_qty;
            delete this.pos.scanned_qty;
        }
