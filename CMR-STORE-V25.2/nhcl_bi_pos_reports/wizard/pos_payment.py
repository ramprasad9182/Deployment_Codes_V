...
class PosPayment(models.TransientModel):
    ...
    def pos_payment_report(self):
        return self.env.ref('nhcl_bi_pos_reports.action_pos_payment_report').report_action(self)
    ...
    def print_excel_report(self):
        ...
        pos_orders = self.env['pos.order'].search(
            [('date_order', '>=', self.start_dt), ('date_order', '<=', self.end_dt),
        ...
        for order in pos_orders:
            if user_currency != order.pricelist_id.currency_id:
                total += order.pricelist_id.currency_id._convert(order.amount_total, user_currency, order.company_id,
                                                                 order.date_order)
            else:
        ...
        for product_details in data.get('products'):
            ...
            worksheet.write(r, 3, str(round(product_details.get('quantity') * product_details.get('price_unit'), 2)),
                            num_style)
        ...
        for payment in payments:
            r = r + 1
            worksheet.write(r, 0, payment['s_name'])
