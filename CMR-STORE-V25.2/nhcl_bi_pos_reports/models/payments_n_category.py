...
class PosOrderForPaymentReport(models.Model):
    ...
    def get_crnt_ssn_payment_pos_order(self, smry, main_cshr, cshr, config_id, curr_session, is_current_session,
                                       pay_st_date, pay_ed_date):
        ...
        if product_ids:
            self.env.cr.execute("""
				...
				WHERE pol.product_id = product.id
					AND product.bi_pos_reports_category = pc.id
