...
class Respartner(models.Model):
    ...
    @api.model
    def _cron_generate_e_bill_token(self):
        """Cron action to auto-generate E-Way Bill token every 6 hours."""
        companies = self.search([
            ('e_bill_api_url', '!=', False),
            ('e_bill_username', '!=', False),
            ('e_bill_password', '!=', False),
        ])
        print("Found %d companies with E-Way Bill credentials." % len(companies), companies.mapped('name'))
        for company in companies:
            try:
                company.generate_token_for_e_bill()
                print("Auto-generated E-Way bill token for company %s (ID: %s)", company.name, company.id)
            except Exception as e:
                print("Cron failed to auto-generate E-Way bill token for company %s (ID: %s): %s", company.name, company.id, str(e))
    ...
    def get_e_bill_header(self):
        headers = {
            ...
            'client_id': self.e_bill_client_id or '', 
            'client_secret': self.e_bill_client_secret or '',
            'gstin': self.vat and self.vat or '',
            'authtoken': self.e_bill_authtoken or '',
            }
        if self.e_bill_environment == 'sandbox':
