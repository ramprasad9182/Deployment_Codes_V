...
class EwayBillDetails(models.Model):
    ...
    def _save_ewaybill_request_json(self, operation, data):
        """Attach an outbound POST JSON body to its invoice or E-way bill."""
        self.ensure_one()
        json_content = data if isinstance(data, str) else json.dumps(data)
        target = self.move_id or self
        document_name = re.sub(
            r"[^A-Za-z0-9_.-]+", "_", self.name or str(self.id)
        )
    ...
    def generate_json_data(self):
        ...
        doc_type = self.doc_type or "INV"
        sub_supply_type = self.sub_supply_type or "1"
        sub_supply_desc = " "

        from_gstin = (company.vat or "").strip()
        to_gstin = (vendor.vat or "").strip()
        if from_gstin and to_gstin and from_gstin == to_gstin:
            if doc_type == "INV":
                doc_type = "CHL"
            if sub_supply_type == "1":
                sub_supply_type = "5"
                sub_supply_desc = "For Own Use"
    ...
    def generate_eway_bill(self):
        ...
        self._save_ewaybill_request_json("generate_eway_bill", data)
        ...
        # Retry once if token error occurred
        if json_data.get("status_cd") == "0":
            status_desc_check = str(json_data.get("status_desc") or "").lower()
            if "token" in status_desc_check or "108" in status_desc_check or "213" in status_desc_check:
                company_id.sudo().generate_token_for_e_bill()
                headers = company_id.sudo().get_e_bill_header()
                self._save_ewaybill_request_json("generate_eway_bill_retry", data)
                response = requests.post(url=auth_url, headers=headers, data=data)
                if response.status_code == 200:
                    json_data = response.json()
        ...
        if json_data.get("status_cd") == "1":
            ...
            err_msg = self.company_id.parse_ewaybill_error(status_desc or json_data)
            if err_msg:
                msg += err_msg + "\n"

        if msg:
            wiz = self.env['message.wizard.ewaybill'].with_context(message=msg, default_name=msg).create({})
            view = self.env.ref('lax_ewaybill.message_wizard')
            context = dict(self._context or {})
            context['message'] = msg
            context['default_name'] = msg
            return {
                'name': title or _("E-Way Bill Message"),
    ...
    def generate_consolidated_ewaybill(self):
        ...
        data = json.dumps(data)
        self._save_ewaybill_request_json("generate_consolidated_ewaybill", data)
        response = requests.post(url=auth_url, headers=headers, data=data)
        json_data = response.json()
        msg = ""
        title = ""
    ...
    def reject_ewbno(self):
        ...
        data = json.dumps(data)
        self._save_ewaybill_request_json("reject_ewbno", data)
        response = requests.post(url=auth_url, headers=headers, data=data)
        json_data = response.json()
    ...
    def get_grouped_item_lines(self):
        self.ensure_one()
        ...
        for line in self.item_ids:
            ...
            # HSN + Product + Price Unit must be same
            key = (hsn, product_id, price_unit)
        ...
        # Total = grouped quantity * price unit
        for values in grouped.values():
            values['total'] = values['qty'] * values['price_unit']
