...
class PosOrder(models.Model):
    ...
    def store_pos_delivery_orders(self):
        ...
        try:
            ...
            # PARTNER LOOKUP & BULK AUTO-CREATION (By Name & Phone - Chunked)
            partner_map = {}
            ...
            chunk_size = 50
            if partner_names:
                for i in range(0, len(partner_names), chunk_size):
                    chunk_names = partner_names[i:i + chunk_size]
                    chunk_phones = partner_phones[i:i + chunk_size] if partner_phones else []

                    partner_domain = ["|", ("name", "in", chunk_names),
                                      ("phone", "in", chunk_phones)] if chunk_phones else [
                        ("name", "in", chunk_names)]
                ...
                if missing_partners_payload:
                    for i in range(0, len(missing_partners_payload), chunk_size):
                        sub_payload = missing_partners_payload[i:i + chunk_size]
                        sub_keys = missing_keys[i:i + chunk_size]
            ...
            existing_res = session.get(
                f"{base_url}/stock.picking/search",
                params={
                    "domain": str([("origin", "in", order_names), ("company_id", "in", company_ids), ]),
                    "fields": "['id','origin']",
                },
                timeout=120,
            ).json()
            ...
            if all_tax_names and all_target_comp_ids:
                for i in range(0, len(all_tax_names), chunk_size):
                    chunk_taxes = all_tax_names[i:i + chunk_size]
                    tax_res = session.get(
        ...
        for order in store_pos_delivery_orders:
            try:
                ...
                if pending_pos_orders and pending_pos_orders.credit_ids:
                    for credit in pending_pos_orders.credit_ids:
                        voucher_number = str(credit.partner_credit_id.voucher_number)
                        picking_credit_lines.append((0, 0, {
                            "partner_credit_id": voucher_number,
                    ...
                    if pos_id:
                        ...
                        if pos_line:
                            ...
                            for t in pos_line.tax_ids:
                                if not t.name:
                                    continue
                                tax_name_key = f"{t.name}-CREDIT"

                                # Check main company tax first, otherwise fallback to store company tax
                                matched_tax_id = tax_map.get(
                                    (tax_name_key, main_company_id)) if main_company_id else None
                                if not matched_tax_id:
                                    matched_tax_id = tax_map.get((tax_name_key, company_id))
                ...
                # SKIP ENTIRE ORDER IF ANY LINE HAS MISSING DEPENDENCY
                if has_missing_line_dependency:
                    continue
                ...
                single_payload = {
                    ...
                    "picking_credit_ids": picking_credit_lines,
                }

                # =========================================================================
                # 3. DIRECT POST PER PICKING
                # =========================================================================
    ...
    def get_pos_customer_exchange_recipt_orders(self):
        ...
        # Fetch up to 10 un-replicated exchange orders
        exchange_orders = self.env["stock.picking"].search(
            [
    ...
    def send_pos_exchange_orders_ho(self):
        self.env['nhcl.initiated.status.log'].create({
            'nhcl_serial_no': self.env['ir.sequence'].next_by_code("nhcl.initiated.status.log"),
            'nhcl_date_of_log': datetime.now(),
            'nhcl_job_name': 'POS Exchange Order Live Sync',
            'nhcl_status': 'success',
            'nhcl_details_status': 'Function Started'
        })
        self.env.cr.commit()
        ...
        self.env['nhcl.initiated.status.log'].create(
            {'nhcl_serial_no': self.env['ir.sequence'].next_by_code("nhcl.initiated.status.log"),
             'nhcl_date_of_log': datetime.now(), 'nhcl_job_name': 'POS Exchange Order Live Sync', 'nhcl_status': 'success',
             'nhcl_details_status': 'Function   Completed'})
        self.env.cr.commit()
        return True
