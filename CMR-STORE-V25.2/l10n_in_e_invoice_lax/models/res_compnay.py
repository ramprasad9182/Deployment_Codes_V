...
class Respartner(models.Model):
    ...
    def onchange_e_inv_environment(self):
        if self.e_inv_environment == 'sandbox':
            self.e_inv_api_url = 'https://apisandbox.whitebooks.in'
            ...
            self.e_inv_api_url = 'https://api.whitebooks.in'
    ...
    def generate_token_for_e_invoice(self):
        ...
        if json_data.get('status_cd') == 'Sucess' and success_data:
            ...
            expiry_time = fields.Datetime.to_datetime(e_inv_tokenexpiry)
            if expiry_time.tzinfo:
                utc_time = expiry_time.astimezone(pytz.utc).replace(tzinfo=None)
            else:
                utc_time = user_tz.localize(expiry_time).astimezone(
                    pytz.utc
                ).replace(tzinfo=None)
            data = {
                'e_inv_sek': e_inv_sek,
