    ...
    async click() {
        ...
        if (confirmed) {
            ...
            if (isNaN(val) || val < 0) {
                this.popup.add(ErrorPopup, {
                    title: _t("Amount Discount Error"),
                    body: _t("Negative discount amount is not allowed!"),
                });
                return;
            }
            if (val > netCartValue) {
                this.popup.add(ErrorPopup, {
                    title: _t("Amount Discount Error"),
                    body: _t("Discount amount cannot be greater than cart value!"),
                });
                return;
            }
            const cashier = this.pos.get_cashier();
            if (cashier && cashier.max_amount_discount > 0 && val > cashier.max_amount_discount) {
                this.popup.add(ErrorPopup, {
                    title: _t("Amount Discount Error"),
                    body: _t(`You cannot give amount discount more than ${cashier.max_amount_discount}.`),
                });
                return;
            }
