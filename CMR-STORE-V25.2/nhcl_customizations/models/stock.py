...
class Picking(models.Model):
    ...
    #         return True
    #
    #     def log_order_failure(order, msg):
    #         try:
    ...
    #
    #         # Index POS Lines for O(1) direct dictionary lookup
    #         pos_line_by_lot = {}
    #         pos_line_by_prod = {}
    #
    ...
    #
    #         # PARTNER LOOKUP & BULK AUTO-CREATION
    #         partner_map = {}
    ...
    #                 params={
    #                     "domain": str([("name", "in", partner_names)]),
    ...
    #
    #             # Bulk create all missing partners in 1 single API call
    ...
    #                 key = (p_name, p_phone)
    #                 if key not in partner_map and p_name not in partner_map and key not in missing_keys:
    ...
    #
    #         # Pre-fetch Employees / Cashiers
    #         line_employees = pos_orders.lines.mapped("employ_id.nhcl_id")
    #         header_employees = pos_orders.mapped("employee_id.nhcl_id")
    #         all_employee_nhcl_ids = list(filter(None, set(line_employees + header_employees)))
    #         cashier_map = {}
    #         if all_employee_nhcl_ids and company_ids:
    #             cashier_res = session.get(
    #                 f"{base_url}/hr.employee/search",
    #                 params={
    #                     "domain": str([
    #                         ("nhcl_id", "in", all_employee_nhcl_ids),
    #                         ("company_id", "in", company_ids),
    #                     ]),
    #                     "fields": "['id','nhcl_id','company_id']",
    #                 },
    #                 timeout=120,
    #             ).json()
    #             for emp in cashier_res.get("data", []):
    #                 emp_nhcl = extract_val(emp.get("nhcl_id"))
    #                 comp_id = extract_id(emp.get("company_id"))
    #                 emp_id = extract_id(emp.get("id"))
    #                 if emp_nhcl is not None and comp_id and emp_id:
    #                     cashier_map[(str(emp_nhcl), comp_id)] = emp_id
    #
    #         # Pre-fetch Taxes
    ...
    #
    #     # =========================================================================
    #     # 2. IN-MEMORY PAYLOAD BUILDING
    #     # =========================================================================
    #
    #     bulk_payload = []
    #     valid_orders = self.env['stock.picking']
    #     already_done_orders = self.env['stock.picking']
    #
    #     for order in store_pos_delivery_orders:
    #         try:
    #             # Quick check if order is already present in HO
    ...
    #
    #             # PARTNER GUARD: Skip order if POS Order has a partner that doesn't exist in HO
    #             ho_partner_id = False
    #             if pending_pos_orders and pending_pos_orders.partner_id:
    #                 p = pending_pos_orders.partner_id
    #                 ho_partner_id = partner_map.get((p.name, p.phone or False)) or partner_map.get(p.name, False)
    ...
    #             if not company_id:
    #                 log_order_failure(order, f"Pos Delivery Order Company Not Found : {order.name}")
    #                 continue
    #
    #             ho_cashier_id = False
    #             if pending_pos_orders and pending_pos_orders.employee_id and pending_pos_orders.employee_id.nhcl_id:
    #                 ho_cashier_id = cashier_map.get((str(pending_pos_orders.employee_id.nhcl_id), company_id), False)
    #
    ...
    #                 log_order_failure(order, f"Pos Delivery Order Location Not Found : {order.name}")
    #                 continue
    #
    #             # Payment lines mapping
    ...
    #
    #                 if pos_id:
    #                     # Fast O(1) Dict Lookup instead of Odoo recordset filter
    ...
    #                         if emp and emp.nhcl_id:
    #                             ho_employee_id = cashier_map.get((str(emp.nhcl_id), company_id), False)
    #                             if not ho_employee_id:
    #                                 log_order_failure(order, f"Pos Delivery Order Employee Not Found : {order.name}")
    #                                 has_missing_line_dependency = True
    #                                 break
    #
    ...
    #                                 log_order_failure(order, f"Pos Delivery Order Tax Not Found : {order.name}")
    #                                 has_missing_line_dependency = True
    #                                 break
    ...
    #                     "applied_program": pos_line.applied_program_id.name if pos_line and pos_line.applied_program_id else None,
    #                     "customer_note": pos_line.customer_note if pos_line else None,
    #                     "product_name": pos_line.full_product_name if pos_line else None,
    #                     "cgst_amount": ((
    #                                                 pos_line.price_subtotal_incl - pos_line.price_subtotal) / 2) if pos_line and pos_line.price_subtotal_incl else None,
    #                     "sgst_amount": ((
    #                                                 pos_line.price_subtotal_incl - pos_line.price_subtotal) / 2) if pos_line and pos_line.price_subtotal_incl else None,
    #                     "badge_id": pos_line.badge_id if pos_line else None,
    #                     "fix_discount_amount": pos_line.fix_discount_amount if pos_line else None,
    #                     "fix_discount_percentage": pos_line.fix_discount_percentage if pos_line else None,
    #                 }
    ...
    #
    #     # Write status for orders that were already synced
    #     if already_done_orders:
    #         already_done_orders.write({"nhcl_replication_status": True})
    #
    #     # =========================================================================
    #     # 3. SINGLE BULK POST API CALL & MASS RECORD UPDATE
    #     # =========================================================================
    #
    #     if bulk_payload:
    #         for payload, order in zip(bulk_payload, valid_orders):
    #             try:
    #                 res_raw = session.post(
    #                     f"{base_url}/stock.picking/create",
    #                     json=[payload],  # Send as single item list
    #                     timeout=300,
    #                 )
    #
    #                 try:
    ...
    #                 except Exception:
    #                     response = {"error": res_raw.text[:300]}
    ...
    #                 else:
    #                     # Extract exact meaningful error returned by Odoo HO
    #                     err_msg = (
    #                             response.get("message")
    #                             or response.get("error")
    #                             or response.get("result", {}).get("message")
    #                             or response.get("reason")
    #                             or f"HTTP {res_raw.status_code}: {res_raw.text[:200]}"
    #                     )
    #                     log_order_failure(
    #                         order,
    #                         f"HO Delivery Sync Failed for {order.name}: {err_msg}"
    #                     )
    #
    #             except Exception as post_err:
    #                 log_order_failure(
    #                     order,
    #                     f"HO Connection Error for {order.name}: {str(post_err)}"
    #                 )
    #
    #     session.close()
    #     return True
        ...
        for order in store_pos_delivery_orders:
            try:
                ...
                if is_success:
                    ...
                    log_order_failure(order, f"HO Single Post Failed : {order.name} ({err_detail})")
    ...
    stock_type = fields.Selection(
        [('advertisement', 'Advertisement'), ('ho_operation', 'HO Operation'), ('data_import', 'Data Import'),
         ...
         ('intra_state', 'Intra State'),('inf','INF'),
    ...
    nhcl_pos_order = fields.Many2one(
        'pos.order',
        string="Same POS Order",
        copy=False,
       )
    ...
    def check_nhcl_pos_order(self):
        for rec in self:
            if rec.nhcl_pos_order and rec.stock_picking_type == 'exchange':
                #if 'refund' in (
                #       rec.nhcl_pos_order.name or '').lower()  or rec.nhcl_pos_order.amount_total <= 0:
                #                    raise ValidationError(_("Refunded orders or refund orders cannot be selected for exchange."))
    ...
    def set_stock_type(self):
        for rec in self:
            ...
            #     raise ValidationError(f"Please Select Same Company.")

    def _check_nh_date_period(self):
        allowed_days = int(
            self.env['ir.config_parameter'].sudo().get_param(
                'nh_inventory.nh_picking_allowed_days',
                default=0
            )
        )
        ...
        start_date = today - timedelta(days=allowed_days)

        for picking in self:
            if not picking.date_time_nh:
                continue

            nh_date = fields.Datetime.to_datetime(
                picking.date_time_nh
            ).date()

            if nh_date < start_date or nh_date > today:
                raise UserError(_(
                    "Exchange period is over.\n\n"
                    "Bill Date: %s\n"
                    "Exchange Period: %s to %s"
                ) % (
                                    nh_date.strftime('%d-%m-%Y'),
    ...
    def nhcl_get_pos_order(self):
        for rec in self:
            ...
            if rec.nhcl_pos_order:
                ...
                rec.date_time_nh = rec.nhcl_pos_order.date_order
                rec.customer_phone = rec.nhcl_pos_order.partner_phone_display
                rec.stock_type = 'ho_operation'
                move_lines = []
                lot_name = False
                # Iterate over each line in the pos order
                for line in rec.nhcl_pos_order.lines.filtered(lambda x: x.is_pos_order_used_line == False):
                    ...
                    move_line_vals = {
                        ...
                        'move_brand_barcode': lots.ref,
                        # 'nhcl_on_cart_without_discount': True if line.reward_id else False
                    }
                    move_lines.append((0, 0, move_line_vals))
                rec.move_ids_without_package = move_lines
                # for reward_line in rec.nhcl_pos_order.lines.filtered(lambda x:x.reward_id):
                #     # if reward_line.reward_id.program_id.rule_ids[0].type_filter=='cart':
                #     move_len = len(rec.move_ids_without_package.filtered(lambda x:not x.nhcl_on_cart_without_discount))
                #     for move in rec.move_ids_without_package.filtered(lambda x:not x.nhcl_on_cart_without_discount):
                #         move.nhcl_on_cart_discount_value = reward_line.price_unit/move_len
                rec.move_ids.move_line_ids.write({'lot_name': lot_name})
                rec._check_nh_date_period()
    ...
    #                 'categ_9': lot_data.category_9.id if lot_data and lot_data.category_9 else False,
    #
    ...
    def nhcl_get_pos_order_from_diff_store(self):
        for rec in self:
            ...
            if rec.store_pos_order:
                ...
                if not ho_id:
                    raise ValidationError(_("HO Store Master configuration not found."))
                ...
                # 1. Fetch POS Order securely with optimized fields filter
                pos_data_url = f"http://{ho_ip}:{ho_port}/api/stock.picking/search"
                pos_domain = json.dumps([('pos_reference', '=', rec.store_pos_order)])
                pos_fields = json.dumps(['id', 'partner_id', 'date_order', 'company_id'])
                pos_response = requests.get(
                    pos_data_url,
                    headers=headers_source,
                    params={'domain': pos_domain, 'fields': pos_fields}
                ).json()

                if not pos_response.get("data"):
                    raise ValidationError(_('POS Order Number does not exist or is already used.'))

                pos_record = pos_response["data"][0]
                pos_order_id = pos_record["id"]

                # 2. Fetch Partner Details
                partner_id = False
                if pos_record.get('partner_id'):
                    partner_remote_id = pos_record['partner_id'][0]['id'] if isinstance(pos_record['partner_id'],
                                                                                        list) else pos_record[
                        'partner_id']
                    partner_url = f"http://{ho_ip}:{ho_port}/api/res.partner/search"
                    partner_domain = json.dumps([('id', '=', partner_remote_id)])
                    partner_fields = json.dumps(['id', 'phone', 'name'])
                    partner_response = requests.get(
                        partner_url,
                        headers=headers_source,
                        params={'domain': partner_domain, 'fields': partner_fields}
                    ).json()
                ...
                def format_odoo_datetime(dt_str):
                    if not dt_str:
                        return False
                    return dt_str.replace('T', ' ').split('.')[0]

                date_order = format_odoo_datetime(pos_record.get("date_order"))

                rec.partner_id = partner_id
                rec.nhcl_invoice_date = date_order
                rec.date_time_nh = date_order
                rec.nhcl_purchased_store = pos_record.get("company_id")[0]['name'] if isinstance(
                    pos_record.get("company_id"), list) else False
                rec.stock_type = 'ho_operation'

                # 3. Process Stock Move Lines safely from HO with optimized fields
                move_line_url = f"http://{ho_ip}:{ho_port}/api/stock.move.line/search"
                move_line_domain = json.dumps([('picking_id', '=', pos_order_id)])
                move_line_fields = json.dumps([
                    'id', 'product_id', 'quantity', 'product_uom_qty', 'lot_name',
                    'internal_ref_lot', 'ref', 'type_product', 'tax_ids_after_fiscal_position',
                    'discount', 'gdiscount', 'disc_lines', 'price_subtotal',
                    'price_subtotal_incl', 'rs_price', 'full_product_name'
                ])
                move_line_response = requests.get(
                    move_line_url,
                    headers=headers_source,
                    params={'domain': move_line_domain}
                ).json()

                if not move_line_response.get("data"):
                    raise ValidationError(_('No stock move lines found for this POS Order.'))

                for line in move_line_response["data"]:
                    # Fetch Product Details
                    product_remote_id = line['product_id'][0]['id'] if isinstance(line.get('product_id'),
                                                                                  list) else line.get('product_id')
                    product_id = False

                    if product_remote_id:
                        prod_url = f"http://{ho_ip}:{ho_port}/api/product.product/search"
                        prod_domain = json.dumps([('id', '=', product_remote_id)])
                        prod_fields = json.dumps(['id', 'barcode', 'name'])
                        prod_response = requests.get(
                            prod_url,
                            headers=headers_source,
                            params={'domain': prod_domain, 'fields': prod_fields}
                        ).json()

                        if prod_response.get("data"):
                            product_data = prod_response["data"][0]
                            product_code = product_data.get("barcode")
                            if product_code:
                                product_id = self.env['product.product'].search([('barcode', '=', product_code)],
                                                                                limit=1)
                            if not product_id:
                                raise ValidationError(
                                    _('Product with barcode %s not found in local inventory.', product_code))

                    # Safe extraction for lot/serial and pricing fields
                    lot_name_val = line.get("lot_name")
                    ...
                    categ_3_id = False
                    categ_8_id = False
                    nhcl_lot_hsn_code = False
                    type_prod_val = False
                    lot_ref_val = False
                    mr_price = 0.0
                    rs_price = 0.0

                    # Check and verify Lot from HO if lot_name exists
                    if lot_name_val:
                        lot_url = f"http://{ho_ip}:{ho_port}/api/stock.lot/search"
                        lot_domain = json.dumps([('name', '=', lot_name_val)])
                        lot_fields = json.dumps([
                            'id', 'name', 'ref', 'type_product', 'nhcl_l10n_in_hsn_code',
                            'sale_tax_ids', 'purchase_tax_ids', 'description_1',
                            'description_8', 'category_3', 'category_8','mr_price','rs_price'
                        ])
                        lot_response = requests.get(
                            lot_url,
                            headers=headers_source,
                            params={'domain': lot_domain, 'fields': lot_fields}
                        ).json()

                        if lot_response.get("data"):
                            lot_data = lot_response["data"][0]
                            lot_ref_val = lot_data.get("ref") or line.get("internal_ref_lot") or line.get("ref")
                            type_prod_val = lot_data.get("type_product")
                            nhcl_lot_hsn_code = lot_data.get("nhcl_l10n_in_hsn_code")
                            rs_price = lot_data.get("rs_price")
                            mr_price = lot_data.get("mr_price")

                            # Helper to safely extract remote value name across different format structures
                            def extract_name(remote_val):
                                if not remote_val:
                                    return False
                                if isinstance(remote_val, dict):
                                    return remote_val.get('name')
                                elif isinstance(remote_val, list) and len(remote_val) > 0:
                                    first_item = remote_val[0]
                                    if isinstance(first_item, dict):
                                        return first_item.get('name')
                                    elif len(remote_val) > 1 and isinstance(remote_val[1], str):
                                        return remote_val[1]
                                elif isinstance(remote_val, str):
                                    return remote_val
                                return False

                            # Process Sale Taxes by Name
                            for sale in lot_data.get("sale_tax_ids", []):
                                tax_name = extract_name(sale)
                                if tax_name:
                                    cleaned_tax_name = tax_name.replace('-CREDIT', '').strip()
                                    tax = self.env['account.tax'].search([
                                        '|', ('name', '=', cleaned_tax_name), ('name', 'ilike', cleaned_tax_name)
                                    ], limit=1)
                                    if tax:
                                        sale_tax_ids.append(tax.id)

                            # Process Purchase Taxes by Name
                            for purchase in lot_data.get("purchase_tax_ids", []):
                                tax_name = extract_name(purchase)
                                if tax_name:
                                    cleaned_tax_name = tax_name.replace('-CREDIT', '').strip()
                                    tax = self.env['account.tax'].search([
                                        '|', ('name', '=', cleaned_tax_name), ('name', 'ilike', cleaned_tax_name)
                                    ...
                                    if tax:
                                        purchase_tax_ids.append(tax.id)

                            # Search description_1 in product.aging.line by name
                            desc_1_name = extract_name(lot_data.get("description_1"))
                            if desc_1_name:
                                desc_1_rec = self.env['product.aging.line'].search([('name', '=', desc_1_name)],
                                                                                   limit=1)
                                desc_1_id = desc_1_rec.id if desc_1_rec else False

                            # Search description_8 in product.attribute.value by name
                            desc_8_name = extract_name(lot_data.get("description_8"))
                            if desc_8_name:
                                desc_8_rec = self.env['product.attribute.value'].search([('name', '=', desc_8_name)],
                                                                                        limit=1)
                                desc_8_id = desc_8_rec.id if desc_8_rec else False

                            # Search category_3 in product.attribute.value by name
                            categ_3_name = extract_name(lot_data.get("category_3"))
                            if categ_3_name:
                                categ_3_rec = self.env['product.attribute.value'].search([('name', '=', categ_3_name)],
                                                                                         limit=1)
                                categ_3_id = categ_3_rec.id if categ_3_rec else False

                            # Search category_8 in product.attribute.value by name
                            categ_8_name = extract_name(lot_data.get("category_8"))
                            if categ_8_name:
                                categ_8_rec = self.env['product.attribute.value'].search([('name', '=', categ_8_name)],
                                                                                         limit=1)
                                categ_8_id = categ_8_rec.id if categ_8_rec else False

                        else:
                            lot_ref_val = line.get("internal_ref_lot") or line.get("ref")
                    else:
                        lot_ref_val = line.get("internal_ref_lot") or line.get("ref")

                    qty_val = line.get("quantity") or line.get("product_uom_qty", 0.0)

                    # Robust Tax Mapping with "-CREDIT" removal
                    tax_id = False
                    tax_field = line.get("tax_ids_after_fiscal_position")
                    if tax_field:
                        tax_info = tax_field[0]
                    ...
                    move_id = self.env['stock.move'].create({
                        'location_dest_id': rec.location_dest_id.id,
                        'location_id': rec.location_id.id,
                        'name': product_id.display_name if product_id else "Move Line",
                        'picking_id': rec.id,
                        'product_id': product_id.id if product_id else False,
                        'product_uom_qty': qty_val,
                        'quantity': qty_val,
                        'nhcl_old_qty': qty_val,
                        'nhcl_tax_ids': [(6, 0, [tax_id])] if tax_id else [],
                        'nhcl_discount': line.get('discount', 0.0),
                        'nhcl_gdiscount': line.get('gdiscount', 0.0),
                        'nhcl_disc_lines': line.get('disc_lines'),
                        'type_product': type_prod_val,
                        "serial_no": lot_name_val,
                        'ref_pos_order_line_id': line.get('id'),
                        'nhcl_price_subtotal': line.get('price_subtotal', 0.0),
                        'nhcl_price_total': line.get('price_subtotal_incl', 0.0),
                        'nhcl_rsp': line.get('rs_price', 0.0),
                        'move_brand_barcode': lot_ref_val,
                    })

                    # Write explicitly on Stock Move Lines
                    for move_line in move_id.move_line_ids:
                        move_line.write({
                            "internal_ref_lot": lot_ref_val,
                            'rs_price': rs_price,
                            'mr_price': mr_price,
                            ...
                            "quantity": qty_val,
                            'nhcl_lot_hsn_code': nhcl_lot_hsn_code,
    ...
    # Promo Updation
    def update_promo_with_grc_serials(self):
        LoyaltyLine = self.env['loyalty.line']
        LoyaltyProgram = self.env['loyalty.program']

        # ----------------------------
        # Collect all lots
        # ----------------------------
        lots = self.move_line_ids.filtered(lambda l: l.lot_id and l.lot_id.description_8).mapped('lot_id')

        if not lots:
            return

        offer_names = list(set(lots.mapped('description_8.name')))

        # ----------------------------
        # Read all programs once
        # ----------------------------
        programs = LoyaltyProgram.search([
            ('name', 'in', offer_names)
        ])
        ...
        # ----------------------------
        for lot in lots:
            if lot.description_8.name not in program_map:
                raise ValidationError(
                    _("No Loyalty Program found for offer '%s' (Lot: %s).")
                    % (lot.description_8.name, lot.name)
                )

        # ----------------------------
        # Existing loyalty lines
        # ----------------------------
        rule_ids = programs.mapped('rule_ids').ids

        existing_lines = LoyaltyLine.search([
            ('loyalty_id', 'in', rule_ids)
        ])

        existing_keys = {
            (l.loyalty_id.id, l.lot_id.id)
        ...
        for lot in lots:
            ...
            promo = program_map[lot.description_8.name]

            for rule in promo.rule_ids:

                serials = set(
                    filter(None, (rule.serial_nos or "").replace(" ", "").split(","))
                )

                if lot.name not in serials:
                    serials.add(lot.name)

                vals = {
                    'serial_nos': ",".join(sorted(serials)),
                }

                if lot.id not in rule.serial_ids.ids:
                    vals.setdefault('serial_ids', []).append((4, lot.id))

                if lot.product_id.id not in rule.product_ids.ids:
                    vals.setdefault('product_ids', []).append((4, lot.product_id.id))
            ...
            reward_vals = {
                'discount_product_ids': [(4, lot.product_id.id)]
            }

            promo.reward_ids.write(reward_vals)

        # ----------------------------
        # Bulk create loyalty lines
        # ----------------------------
        if create_vals:
            LoyaltyLine.create(create_vals)
    ...
    def button_validate(self):
        ...
        for rec in self:
            ...
            #                 'category_9': move_line.categ_9.id if move_line.categ_9 else False,
            ...
            if rec.picking_type_id.code == 'incoming' and rec.stock_type in ['data_import','inf']:
                # ---------- Barcode validation ----------
            ...
            if rec.stock_picking_type == 'receipt' and rec.picking_type_code == 'incoming' and rec.stock_type not in ['data_import','inf']:
                ...
                for line in rec.stock_verification_ids:
                    move_line_vals.append({
                        ...
                        'categ_1': line.categ_1.id,
                        'categ_2': line.categ_2.id,
                        'categ_3': line.categ_3.id,
                        'categ_4': line.categ_4.id,
                        'categ_5': line.categ_5.id,
                        'categ_6': line.categ_6.id,
                        'categ_7': line.categ_7.id,
                        'categ_8': line.categ_8.id,
                        'categ_9': line.categ_9.id,

                        'descrip_1': line.descrip_1.id,
                        'descrip_2': line.descrip_2.id,
                        'descrip_3': line.descrip_3.id,
                        'descrip_4': line.descrip_4.id,
                        'descrip_5': line.descrip_5.id,
                        'descrip_6': line.descrip_6.id,
                        'descrip_7': line.descrip_7.id,
                        'descrip_8': line.descrip_8.id,
                    })
    ...
    def _onchange_verify_barcode(self):
        if self.verify_barcode:
            if (not self.backorder_id or (self.backorder_id and self.stock_picking_delivery_ids)):
            ...
            gs1_pattern = r'01(\d{14})21([A-Za-z0-9]+)'
            # ean13_pattern = r'^(\d{1,21})$'
            custom_serial_pattern = r'(^([A-Za-z]+)[A-Za-z0-9_/\- ]*\d.*$)'
            brand_lines = self.stock_verification_ids.filtered(
                lambda x: x.stock_product_barcode == barcode
                          and x.type_product == 'brand'
            )
            ...
            if re.match(gs1_pattern, barcode):
                ...
                matched_line = self.stock_verification_ids.filtered(lambda x: x.stock_serial == code)[:1]
                if not matched_line:
            ...
            # ---------------- DIRECT BRAND PRODUCT BARCODE MATCH ----------------
            elif brand_lines:

                tracking_type = brand_lines[0].stock_product_id.tracking

                if tracking_type == 'serial':

                    serial_line = brand_lines.filtered(
                        lambda x: x.stock_status == 'un_matched'
                    )[:1]

                    if not serial_line:
                        raise ValidationError(_("All serials already matched."))

                    if self.receipt_lot_qty != 0:
                        raise ValidationError(_("You should not enter qty."))
                    ...
                    if self.receipt_lot_qty <= 0:
                        raise ValidationError(_("You should enter qty."))
                    ...
                    if self.receipt_lot_qty > total_available:
                        raise ValidationError(
                            _("You have given %s more than %s.")
                            % (self.receipt_lot_qty, total_available)
                        )
                    ...
                    for line in brand_lines:
                        ...
                        allowed = min(remaining, qty_to_allocate)

                        line.stock_actual_qty += allowed

                        if line.stock_actual_qty == line.stock_qty:
                            line.stock_status = 'matched'

                        qty_to_allocate -= allowed
    ...
    #         ean13_pattern = r'^(\d{1,21})$'
    #         custom_serial_pattern = r'(^([A-Za-z]+)[A-Za-z0-9_/\- ]*\d.*$)'
    #         matched_lines = self.stock_verification_ids.filtered(lambda x: x.stock_product_barcode == barcode and x.type_product == 'brand' and x.stock_status == 'un_matched')
    #
    #         if re.match(gs1_pattern, barcode):
    #             # GS1 Barcode
    #             product_barcode = re.match(gs1_pattern, barcode).group(1)
    ...
    def regenerate_detail_operation(self):
        for picking in self:
            ...
            # Delete existing move lines and moves
            picking.move_line_ids.unlink()
            picking.move_ids.unlink()
            # Create move lines directly from POS order lines
...
class StockMove(models.Model):
    ...
    def _action_done(self, cancel_backorder=False):
        for move in self:
            if move.product_id.tracking in ['serial', 'lot'] and move.picking_id.stock_type in ['data_import','inf']:
...
class StockMoveLine(models.Model):
    ...
    categ_9 = fields.Many2one('product.attribute.value', string="Variants",
                              domain=[('attribute_id.name', '=', 'Variants')])
        ...
        for rec in self:
            if rec.picking_id.picking_type_id.stock_picking_type != 'receipt':
                if rec.lot_id:
                    ...
                    rec.categ_9 = rec.lot_id.category_9
    ...
    def _create_and_assign_production_lot(self):
        ...
        is_receipt = self.picking_id.stock_picking_type == 'receipt'
        is_exchange = self.picking_id.stock_picking_type == 'exchange'
        if self.picking_id.picking_type_id.code == 'incoming':
            for rec in self:
                if is_receipt:
                    missing_fields = []
                    if not rec.type_product:
                        missing_fields.append('Type Product')
                    if not rec.internal_ref_lot:
                        missing_fields.append('Internal Reference / Lot Reference')

                    if not rec.nhcl_lot_hsn_code:
                        missing_fields.append('HSN Code')

                    if not rec.sale_tax_ids:
                        missing_fields.append('Sale Taxes')

                    if not rec.purchase_tax_ids:
                        missing_fields.append('Purchase Taxes')

                    if rec.mr_price < 0.1:
                        missing_fields.append('MR Price should be greater than or equal to 0.1')

                    if not rec.cost_price:
                        missing_fields.append('Cost Price')

                    if rec.rs_price < 0.1:
                        missing_fields.append('RS Price should be greater than or equal to 0.1')

                    if missing_fields:
                        raise ValidationError(_(
                            "The following fields are mandatory for Receipt:\n\n%s"
                        ) % '\n'.join(
                            '- %s' % field for field in missing_fields
                        ))
                elif is_exchange:
                    missing_fields = []
                    if not rec.type_product:
                    ...
                    if not rec.purchase_tax_ids:
                        missing_fields.append('Purchase Taxes')
                    if not rec.mr_price:
                        missing_fields.append('MR Price')
                    if not rec.rs_price:
                        missing_fields.append('RS Price')
...
class StockLot(models.Model):
    ...
    category_8 = fields.Many2one('product.attribute.value', string='Design', copy=False,
                                 domain=[('attribute_id.name', '=', 'Design')])
    category_9 = fields.Many2one('product.attribute.value', string="Variants",
                              domain=[('attribute_id.name', '=', 'Variants')])
    ...
    @api.onchange('category_9')
    def updating_line_to_lot_category_9(self):
        detail = self.env['stock.move.line'].search([('lot_id.name', '=', self.name)])
        detail.categ_9 = self.category_9
    ...
    def _split_batches(self, data, batch_size=500):
        for i in range(0, len(data), batch_size):
            yield data[i:i + batch_size]

    def sync_price_from_remote(self):
        session = requests.Session()

        store = self.env['nhcl.ho.store.master'].search([], limit=1)
        if not store:
            return True

        headers = {
            'api-key': store.nhcl_api_key,
            'Content-Type': 'application/json',
        }

        base_url = f"http://{store.nhcl_terminal_ip}:{store.nhcl_port_no}/api"
        search_url = f"{base_url}/stock.lot/search"
        ...
        field_attribute_map = {
            ...
            'category_3': 'Brand',
            'category_4': 'Pattern',
            'category_5': 'Border Type',
            'category_6': 'Border Size',
            'category_7': 'Size',
            'category_8': 'Design',
            'category_9': 'Variants',
            'description_2': 'Range',
            'description_3': 'Collection',
            'description_4': 'Fabric',
            'description_5': 'Exclusive',
            'description_6': 'Print',
            'description_7': 'Days Ageing',
            'description_8': 'Offer',
        }

        try:
            lot_names = list(set(self.mapped('name')))
            remote_lot_map = {}

            # ---------------------------------------------------------
            # Fetch remote lots
            # ---------------------------------------------------------
            for batch in self._split_batches(lot_names):
                domain = [
                    ('name', 'in', batch),
                ...
                remote_lot_map.update({
                    rec['name']: rec
                    for rec in response.json().get('data', [])
                })

            ProductAttributeValue = self.env['product.attribute.value']
            ProductAgingLine = self.env['product.aging.line']
            ...
            for remote in remote_lot_map.values():
                for field, attribute in field_attribute_map.items():
                    data = remote.get(field)

                    if data:
                        value_name = data[0].get('name')
                        if value_name:
                            attribute_names.add((attribute, value_name))

            if attribute_names:
                attribute_records = ProductAttributeValue.search([
                    ('name', 'in', list(set(name for _, name in attribute_names))),
                ])

                for value in attribute_records:
                    key = (value.attribute_id.name, value.name)
                    attribute_cache[key] = value.id
            ...
            for remote in remote_lot_map.values():
                data = remote.get('description_1')

                if data:
                    aging_name = data[0].get('name')
                    if aging_name:
                        aging_names.add(aging_name)

            aging_cache = {}

            if aging_names:
                aging_records = ProductAgingLine.search([
            ...
            # =========================================================
            #
            # Remote response is expected like:
            #
            # "sale_tax_ids": [
            #     {"id": 10, "name": "GST 5%"},
            #     {"id": 11, "name": "GST 12%"}
            ...
            # ]
            #
            # Remote IDs are NOT used.
            # Local tax is identified using name + type_tax_use.
            #
            sale_tax_names = set()
            purchase_tax_names = set()

            for remote in remote_lot_map.values():

                for tax in remote.get('sale_tax_ids') or []:
                    ...
                    if tax_name:
                        sale_tax_names.add(tax_name)

                for tax in remote.get('purchase_tax_ids') or []:
                    if isinstance(tax, dict):
                        tax_name = tax.get('name')
                    else:
                        tax_name = tax
                    ...
                    if tax_name:
                        purchase_tax_names.add(tax_name)

            tax_cache = {}

            # Sale taxes
            if sale_tax_names:
                sale_taxes = AccountTax.search([
                    ('name', 'in', list(sale_tax_names)),
                    ('type_tax_use', '=', 'sale'),
                ])

                for tax in sale_taxes:
                    tax_cache[('sale', tax.name)] = tax.id

            # Purchase taxes
            if purchase_tax_names:
                purchase_taxes = AccountTax.search([
                    ('name', 'in', list(purchase_tax_names)),
                    ('type_tax_use', '=', 'purchase'),
                ])

                for tax in purchase_taxes:
                    tax_cache[('purchase', tax.name)] = tax.id
            ...
            for lot in self:
                remote = remote_lot_map.get(lot.name)

                if not remote:
                    continue

                vals = {
                    'rs_price': remote.get('rs_price'),
                    'mr_price': remote.get('mr_price'),
                    'cost_price': remote.get('cost_price'),
                    'nhcl_lot_hsn_code': (
                            remote.get('nhcl_l10n_in_hsn_code')
                            or remote.get('nhcl_lot_hsn_code')
                    ),
                }
                ...
                # Product Aging
                # -----------------------------------------------------
                aging_data = remote.get('description_1')

                if aging_data:
                    aging_name = aging_data[0].get('name')

                    if aging_name:
                        aging_id = aging_cache.get(aging_name)

                        if aging_id:
                            vals['description_1'] = aging_id
                        else:
                            missing_values.add(
                                f"Product Aging : {aging_name}"
                            )
                ...
                for field, attribute in field_attribute_map.items():
                    ...
                    if value_id:
                        ...
                        missing_values.add(
                            f"{attribute} : {value_name}"
                        )

                # -----------------------------------------------------
                # SALE TAXES
                # -----------------------------------------------------
                if 'sale_tax_ids' in remote:
                    ...
                    for tax in remote.get('sale_tax_ids') or []:
                        ...
                        tax_id = tax_cache.get(
                            ('sale', tax_name)
                        )

                        if tax_id:
                            sale_tax_ids.append(tax_id)
                        else:
                            missing_values.add(
                                f"Sale Tax : {tax_name}"
                            )
                ...
                if 'purchase_tax_ids' in remote:
                    ...
                    for tax in remote.get('purchase_tax_ids') or []:
                        ...
                        if tax_id:
                            purchase_tax_ids.append(tax_id)
                        else:
                            missing_values.add(
                                f"Purchase Tax : {tax_name}"
                            )

                    vals['purchase_tax_ids'] = [
                        (6, 0, purchase_tax_ids)
                    ]
            ...
            if missing_values:
                raise ValidationError(_(
                    "The following values are missing in the "
                    "local database:\n\n%s"
                ) % "\n".join(sorted(missing_values)))

            _logger.info(
                "Price sync completed. Updated %s lots.",
                updated_count
            )
            ...
            raise ValidationError(
                _("Unable to connect to the HO server.\n%s") % e
            )
...
class StockBackorderConfirmation(models.TransientModel):
    ...
    def process(self):
        ...
        for pick_id in self.pick_ids:
            if pick_id.picking_type_id.code == 'incoming' and pick_id.stock_picking_type == 'receipt' and pick_id.is_confirm == False:
                ...
                'data_import','inf'] and pick_id.is_confirm == False:
    ...
    def process_cancel_backorder(self):
        for pick_id in self.pick_ids:
            if pick_id.picking_type_id.code == 'incoming' and pick_id.stock_type in ['data_import','inf'] and pick_id.is_confirm == False:
                pick_id.move_ids.auto_generate_serial_numbers()
                pick_id.is_confirm = True
...
class StockVerification(models.Model):
    ...
    stock_qty = fields.Float(string='Qty', copy=False, digits='Product Unit of Measure')
    stock_actual_qty = fields.Float(string='Act Qty', copy=False, digits='Product Unit of Measure')
    ...
    def _check_actual_qty_not_greater(self):
        ...
        for rec in self:
            if float_compare(rec.stock_actual_qty, rec.stock_qty, precision_digits=precision) == 1:
                raise ValidationError(
                    _("Actual Quantity cannot be greater than Available Quantity.")
                )
...
class StockPickingBatch(models.Model):
    ...
    def _compute_transfer_type(self):
        for batch in self:
            ...
            elif batch.stock_picking_type in ['return', 'damage','regular']:
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            operation = operation_map.get(picking_type.stock_picking_type)
            if operation:
                sequence = (
                        self.env['ir.sequence']
                        .with_company(company_id)
                        .next_by_code('picking.batch.return') or '/'
                )
                vals['name'] = f"{company.company_short_code}{operation}{sequence}"
            else:
                vals['name'] = (
                        self.env['ir.sequence']
                        .with_company(company_id)
                        .next_by_code('picking.batch') or '/'
                )
        res = super().create(vals_list)
        for batch in res.filtered(lambda b: b.stock_picking_type == 'receipt'):
            batch.picking_ids.write({
                'nhcl_batch_number': batch.name
            })
...
class StockVerificationImport(models.TransientModel):
    ...
    def action_import(self):
        ...
        if (not self.stock_picking_id.backorder_id or (
                self.stock_picking_id.backorder_id and self.stock_picking_id.stock_picking_delivery_ids)):
        ...
        verification_lines = self.env['stock.verification'].search([
            ('stock_picking_id', '=', self.stock_picking_id.id)
        ])

        verification_lines.write({
            'stock_status': 'un_matched',
            'stock_actual_qty': 0,
        })
        ...
        for row_index, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
            ...
            if unbrand_line:
                ...
                if not brand_line:
                    raise ValidationError(
                        _("No verification line found for barcode '%s' in row %s.") % (raw_code, row_index)
                    )
    ...
    #         raise UserError(_("Please upload an Excel file."))
    #
    #     if (not self.stock_picking_id.backorder_id or (self.stock_picking_id.backorder_id and self.stock_picking_id.stock_picking_delivery_ids)):
    #         if not any(line.status == 'matched' for line in self.stock_picking_id.stock_picking_delivery_ids):
    #             raise ValidationError(_("You cannot proceed. Please scan at least one bale barcode first."))
    #
    #
