        ...
        for company, lines in lines_by_company.items():
            for line in lines.with_company(company):
                ...
                if line.product_id:
                    ...
                    # Source tax comes from Lot
                    taxes = line.lot_ids.sale_tax_ids._filter_taxes_by_company(
                        company
                    )
                    ...
                    if taxes:
                        ...
                        if line.discount > 0:
                            effective_price = line.price_unit * (
                                    1 - line.discount / 100.0
                            )
                        else:
                            effective_price = line.price_unit * (
                                    1 - line.order_id.disco / 100.0
                            )

                        selected_tax = self.env['account.tax']

                        # Find general GST slab
                        for tax in taxes:

                            # IGST has no min/max, so ignore it here
                            if not tax.min_amount and not tax.max_amount:
                                continue

                            min_amount = tax.min_amount or 0.0
                            max_amount = tax.max_amount or 0.0

                            if (
                                    effective_price >= min_amount
                                    and (
                                    not max_amount
                                    or effective_price <= max_amount
                            )
                            ):
                                selected_tax = tax
                                break
                ...
                # Fiscal position should map general GST -> IGST
                ...
                # Important: explicitly assign mapped IGST
                line.tax_id = [(6, 0, result.ids)]
