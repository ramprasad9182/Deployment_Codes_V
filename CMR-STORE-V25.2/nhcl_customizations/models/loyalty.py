...
class LoyaltyReward(models.Model):
    ...
    def action_import_serials(self):
        ...
        for record in self:
            ...
            if record.reward_type not in ['product', 'discount_on_product']:
                raise UserError(
                    "Please select Reward Type as 'Free Product' or 'Discounted Product' before uploading."
                )

            if not record.serial_import:
                raise UserError("Please upload a file.")

            record.stock_lot_ids = [(5, 0, 0)]
            ...
            record.stock_lot_ids = [(6, 0, stock_lots.ids)]
