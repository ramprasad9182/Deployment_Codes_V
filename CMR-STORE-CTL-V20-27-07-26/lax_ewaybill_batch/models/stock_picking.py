    ...
    def action_create_eway_bill(self):
        ...
        for picking in pickings:
            for move in picking.move_ids_without_package:
                ...
                hsn_code = sale_line.lot_ids.nhcl_lot_hsn_code or ''
        ...
        # ----------------------------------------------------
        # Load HSN Masters once
        # ----------------------------------------------------
        hsn_codes = list({key[1] for key in grouped_lines if key[1]})

        hsn_map = {
            h.hsn_code: h
            for h in self.env['hsn.code.master'].search([
        ...
        for (product_id, hsn_code, price_unit), values in grouped_lines.items():
            ...
            # Same company -> GST Taxes
            if first_picking.company_id == self.company_id:
                company_taxes = product.taxes_id.filtered(
                    lambda t: t.company_id == self.company_id
                )
            # Different company -> IGST Taxes
            else:
                company_taxes = product.igst_sale_tax_id.filtered(
                    lambda t: t.company_id == self.company_id
                )
            ...
            if len(company_taxes) == 1:
                ...
                tax = company_taxes.filtered(
                    lambda t:
                    (t.min_amount or 0.0)
                    <= price_unit <=
                    (t.max_amount or float('inf'))
                )[:1]
