...
class EwayBillDetails(models.Model):
    ...
    def generate_json_data(self):
        ...
        final_data_dict = {
            ...
            "fromTrdName": company.name or "",
            "fromAddr1": company.street or "",
            "fromAddr2": company.street2 or "",
            "fromPlace": company.city or "",
            "actFromStateCode": int(company.state_id.l10n_in_tin) if company.state_id else 0,
            "fromPincode": int(company.zip) if company.zip else 0,
            "fromStateCode": int(company.state_id.l10n_in_tin) if company.state_id else 0,
            "toGstin": vendor.vat or "URP",
            "toTrdName": vendor.name or "",
            "toAddr1": vendor.street or "",
            "toAddr2": vendor.street2 or "",
            "toPlace": vendor.city or "",
            "toPincode": int(vendor.zip) if vendor.zip else 0,
            "actToStateCode": int(vendor.state_id.l10n_in_tin) if vendor.state_id else 0,
            "toStateCode": int(vendor.state_id.l10n_in_tin) if vendor.state_id else 0,
            "transactionType": int(self.transaction_type or 1),
            "dispatchFromGSTIN": company.vat or "",
            "dispatchFromTradeName": company.name or "",
            "shipToGSTIN": vendor.vat or "URP",
            "shipToTradeName": vendor.name or "",
    ...
    def generate_eway_bill(self):
        ...
        if not company_id.e_bill_api_url:
            raise UserError(_("Please set the E-way Bill API URL for %s" % company_id.name))
        ...
        auth_url += "?email=%s" % urllib.parse.quote(company_id.e_bill_email or '')
        auth_url += "&irp=%s" % 'NIC1'
    ...
    def get_ewaybill(self):
        if not self.company_id.e_bill_api_url:
            raise UserError(_("Please set the E-way Bill API URL for %s" % self.company_id.name))
        ...
        url += "?email=%s" % urllib.parse.quote(self.company_id.e_bill_email or '')
