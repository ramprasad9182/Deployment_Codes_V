...
class Picking(models.Model):
    ...
    def store_pos_delivery_orders(self):
        ...
        # =========================================================================
        def extract_id(val):
            """Extract integer/primitive ID from list, dict, or scalar."""
        ...
        def extract_val(val):
            """Extract string/primitive value from list, dict, or scalar."""
            if isinstance(val, list) and val:
                val = val[0]
            if isinstance(val, dict):
        ...
        # =========================================================================
        # 1. BULK PRE-FETCHING & MAPPINGS (PRE-CHECK PHASE)
        # =========================================================================
        ...
        # Pre-fetch POS Orders from local DB
        order_origins = list(
            filter(None, store_pos_delivery_orders.mapped("origin"))
        )
        pos_orders = self.env["pos.order"].search([("name", "in", order_origins)])
        pos_order_map = {p.name: p for p in pos_orders}

        # -------------------------------------------------------------------------
        # PARTNER LOOKUP & AUTO-CREATION (SIMPLIFIED BY NAME & PHONE)
        # -------------------------------------------------------------------------
        partner_map = {}
        ...
        if partner_names:
            ...
            for p in partner_res.get("data", []):
                ...
                if p_name and p_id:
                    ...
                    partner_map[p_name] = p_id  # Fallback lookup by name only

            # 2. Create missing partners in HO
            for partner in local_partners:
                p_name = partner.name
                p_phone = partner.phone or False
                key = (p_name, p_phone)

                if key not in partner_map and p_name not in partner_map:
                    create_res = session.post(
                        f"{base_url}/res.partner/create",
                        json=[{"name": p_name, "phone": p_phone}],
                        timeout=20,
                    ).json()

                    c_id = create_res.get("create_id") or create_res.get("ids")
                    if isinstance(c_id, list) and c_id:
                        c_id = c_id[0]

                    if c_id:
                        ho_p_id = extract_id(c_id)
                        partner_map[key] = ho_p_id
                        partner_map[p_name] = ho_p_id

        # Pre-fetch Store Companies
        company_names = list(
            set(store_pos_delivery_orders.mapped("company_id.name"))
        )
        ...
        for c in company_res.get("data", []):
            c_name = extract_val(c.get("name"))
            c_id = extract_id(c.get("id"))
            if c_name and c_id:
        ...
        existing_origins = {
            p["origin"]
            for p in existing_res.get("data", [])
            if p.get("origin")
        }

        # Pre-fetch Picking Types
        picking_type_codes = list(
            set(
                store_pos_delivery_orders.mapped(
        ...
        source_loc_types = list(
            set(store_pos_delivery_orders.mapped("location_id.cmr_location_type"))
        )
        source_loc_map = {}
        ...
        dest_names = list(
            set(
                store_pos_delivery_orders.mapped("location_dest_id.complete_name")
            )
        )
        dest_loc_map = {}
        if dest_names:
        ...
        # Pre-fetch Products
        all_product_nhcl_ids = list(
            set(
                store_pos_delivery_orders.move_line_ids_without_package.mapped(
                    "product_id.nhcl_id"
                )
            )
        )
        ...
        all_employee_nhcl_ids = list(
            filter(
                None,
                set(
                    pos_orders.lines.mapped("employ_id.nhcl_id")
                    + pos_orders.lines.mapped("employ_id.nhcl_id")
                ),
            )
        )
        ...
        tax_map = {}
        all_target_comp_ids = list(
            set(company_ids + ([main_company_id] if main_company_id else []))
        )
        ...
        all_payment_method_names = list(
            filter(
                None,
                set(pos_orders.payment_ids.mapped("payment_method_id.name")),
            )
        )
        payment_method_map = {}
        ...
        for order in store_pos_delivery_orders:
            try:
                ...
                # Get matched HO Partner ID
                ho_partner_id = False
                if pending_pos_orders and pending_pos_orders.partner_id:
                    p = pending_pos_orders.partner_id
                    p_name = p.name
                    p_phone = p.phone or False
                    ho_partner_id = partner_map.get((p_name, p_phone)) or partner_map.get(p_name, False)
                ...
                picking_type_key = (
                    order.picking_type_id.stock_picking_type,
                    company_id,
                )
                picking_type_ho_id = picking_type_map.get(picking_type_key)
                ...
                source_key = (order.location_id.cmr_location_type, company_id)
                location_id = source_loc_map.get(source_key)
                dest_id = dest_loc_map.get(order.location_dest_id.complete_name)

                if not location_id or not dest_id:
                    msg = f"Pos Delivery Order Locations Not Found : {order.location_id.name},{order.location_dest_id.complete_name}"
                    ho_id.create_cmr_transaction_replication_log(
                        "POS Delivery Order",
                        order.id,
                        order.name,
                ...
                # Mark as done locally if already present in HO
                if order.name in existing_origins:
                    order.write({"nhcl_replication_status": True})
                    self.env.cr.commit()
                    continue

                # Build One2many Payment Lines
                ...
                if pending_pos_orders and pending_pos_orders.payment_ids:
                    for pay in pending_pos_orders.payment_ids:
                        if not pay.payment_method_id:
                            continue
                        pm_name = pay.payment_method_id.name
                        ho_pm_id = payment_method_map.get(pm_name)
                        if not ho_pm_id:
                            msg = f"Pos Delivery Order Payment Method Not Found : {pm_name}"
                            ho_id.create_cmr_transaction_replication_log(
                                "POS Delivery Order",
                                order.id,
                                order.name,
                ...
                # Check company state for state-dependent tax mapping
                company_state_name = (
                    order.company_id.state_id.name
                    if order.company_id and order.company_id.state_id
                    else ""
                )
                ...
                for line in order.move_line_ids_without_package:
                    ...
                    if pending_pos_orders:
                        matched_pos_line = False

                        if line.lot_id:
                            matched_pos_line = (
                                pending_pos_orders.lines.filtered(
                                    lambda l: l.product_id == line.product_id
                                              and any(
                                        lot.lot_name == line.lot_id.name
                                        for lot in l.pack_lot_ids
                                    )
                                )
                            )

                        if not matched_pos_line:
                            matched_pos_line = (
                                pending_pos_orders.lines.filtered(
                                    lambda l: l.product_id == line.product_id
                                )
                        ...
                        if matched_pos_line:
                            pos_line = matched_pos_line[0]

                            # Cashier / Employee Lookup & Strict Validation
                            emp = pos_line.employ_id or pos_line.employee_id
                            if emp and emp.nhcl_id:
                                ho_employee_id = cashier_map.get(
                                    (str(emp.nhcl_id), company_id), False
                                )
                                if not ho_employee_id:
                                    msg = f"Pos Delivery Order Employee Not Found : {emp.name}"
                                    ho_id.create_cmr_transaction_replication_log(
                                        "POS Delivery Order",
                                        order.id,
                                        order.name,
                            ...
                            # Tax Lookup with State-dependent Rule & Strict Validation
                            target_tax_comp_id = (
                                company_id
                                if company_state_name != "Andhra Pradesh"
                                else main_company_id
                            )

                            for t in pos_line.tax_ids:
                                if not t.name:
                                    continue
                                credit_tax_name = f"{t.name}-CREDIT"
                                matched_tax_id = tax_map.get(
                                    (credit_tax_name, target_tax_comp_id)
                                )

                                if not matched_tax_id:
                                    msg = f"Pos Delivery Order Tax Not Found : {credit_tax_name}"
                                    ho_id.create_cmr_transaction_replication_log(
                                        "POS Delivery Order",
                                        order.id,
                                        order.name,
                    ...
                    move_line_vals = {
                        ...
                        "total_discount": (
                            pos_line.total_discount
                            if pos_line and pos_line.total_discount
                            else 0.0
                        ),
                        "discount_percentage": (
                            pos_line.discount_percentage
                            if pos_line and pos_line.discount_percentage
                            else 0.0
                        ),
                        "total_reward_discount": (
                            pos_line.total_reward_discount
                            if pos_line and pos_line.total_reward_discount
                            else 0.0
                        ),
                        "gdiscount": (
                            pos_line.gdiscount
                            if pos_line and pos_line.gdiscount
                            else 0.0
                        ),
                        "price_subtotal": (
                            pos_line.price_subtotal
                            if pos_line and pos_line.price_subtotal
                            else 0.0
                        ),
                        "price_subtotal_incl": (
                            pos_line.price_subtotal_incl
                            if pos_line and pos_line.price_subtotal_incl
                            else 0.0
                        ),
                        "applied_program": (
                            pos_line.applied_program_id.name
                            if pos_line and pos_line.applied_program_id
                            else None
                        ),
                        "customer_note": (
                            pos_line.customer_note
                            if pos_line and pos_line.customer_note
                            else None
                        ),
                    }
                ...
                # Append to single batch payload
                bulk_payload.append(
                    {
                        ...
                        "origin": order.name,
                        "pos_reference": (
                            pending_pos_orders.pos_reference
                            if pending_pos_orders
                            else False
                        ),
                        "pos_name": (
                            pending_pos_orders.name
                            if pending_pos_orders
                            else False
                        ),
                        "date_order": (
                            pending_pos_orders.date_order.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            )
                            if pending_pos_orders and pending_pos_orders.date_order
                            else False
                        ),
                        "date_done": (
                            order.date_done.strftime("%Y-%m-%d %H:%M:%S")
                            if order.date_done
                            else False
                        ),
                        "scheduled_date": (
                            order.scheduled_date.strftime("%Y-%m-%d %H:%M:%S")
        ...
        if bulk_payload:
            try:
                ...
                if response.get("success"):
                    ...
                    err_msg = response.get("message", "Bulk POST failed")
                    for order in valid_orders:
                        ho_id.create_cmr_transaction_replication_log(
                            "POS Delivery Order",
                            order.id,
...
class StockLot(models.Model):
    ...
    def sync_price_from_remote(self):
        ...
        # Mapping between field and attribute name
        field_attribute_map = {
            'category_1': 'Color',
            'category_2': 'Fit',
            'category_3': 'Brand',
        ...
        try:
            ...
            for lot in self:
                ...
                vals = {
                    ...
                    'nhcl_lot_hsn_code': remote.get('nhcl_l10n_in_hsn_code') or remote.get('nhcl_lot_hsn_code'),
                }

                # ---------------------------------------------------------
                # Product Aging (description_1)
                # ---------------------------------------------------------
                if remote.get('description_1'):
                    aging_name = remote['description_1'][0]['name']

                    aging = ProductAgingLine.search([
                        ('name', '=', aging_name)
                    ], limit=1)

                    if aging:
                        vals['description_1'] = aging.id
                    else:
                        missing_values.add(f"Product Aging : {aging_name}")
                ...
                for field, attribute in field_attribute_map.items():
                    ...
                    if value:
                        ...
                        missing_values.add(f"{attribute} : {value_name}")
            ...
            if missing_values:
                raise ValidationError(_(
                    "The following values are missing in the local database:\n\n%s"
                ) % "\n".join(sorted(missing_values)))

        except requests.exceptions.RequestException as e:
            _logger.exception("Sync failed: %s", e)
            raise ValidationError(_("Unable to connect to the HO server.\n%s") % e)
