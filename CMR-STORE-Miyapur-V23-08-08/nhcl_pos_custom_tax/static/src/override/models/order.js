...
patch(Order.prototype, {
    ...
    _isRewardProductPartOfRuleSerial(reward, applicableProducts, currentline) {
        ...
        const totalProductValue = eligibleLines.reduce((sum, line) => sum + line.get_price_with_tax_before_discount(), 0);
    ...
    _getDiscountableOnSpecific(reward) {
        ...
        for (const line of linesToDiscount) {
            if (reward.buy_with_reward_price === "yes") {
                ...
                // Standard percentage discount: tagging was already done in Pass 1.
                // The discount value will be applied in _getRewardLineValuesDiscount.
            }
            ...
            if (priceWithTax) {
                discountablePerTax[taxKey] +=
                    line.get_all_prices().priceWithoutTaxBeforeDiscount *
    ...
    _getRewardLineValuesDiscount(args) {
        ...
        if (reward.buy_with_reward_price === 'no') {
            ...
            const discountPercent = 100 * discountFactor;
